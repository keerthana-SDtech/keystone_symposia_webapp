...
const getAutoCompleteValue = (field: FormField, formId?: string) => {
    ...
    if (format === 'email' || name.includes('email')) return "email";
...
export const DynamicForm = forwardRef<DynamicFormRef, DynamicFormProps>(({ config, onSubmit, isSubmitting, hideSubmitButton, id, activeSectionId, onActiveSectionValidChange, initialValues }, ref) => {
    ...
    useEffect(() => {
        if (initialValues && Object.keys(initialValues).length > 0) {
            ...
            config.forEach(section => {
                extractFieldNames(section).forEach(name => {
                    merged[name] = initialValues[name] ?? "";
                });
            });
            reset(merged);
        }
    ...
    useImperativeHandle(ref, () => ({
        ...
        validateAll: async () => {
            ...
            for (const section of config) {
                ...
                const fieldNames = extractFieldNames(section);
                result[sectionId] = fieldNames.filter(name => !!errorsRef.current[name]).length;
            }
            return result;
        }
